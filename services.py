...
def _ensure_prepared():
    ...
    # Zwracamy post_id do sortowania
    _PREPARED['GET_TIMELINE'] = sess.prepare("SELECT author_username, content, post_id FROM timeline_by_user WHERE username =? LIMIT 50")
    ...
    _PREPARED['GET_FOLLOWERS_COUNT'] = sess.prepare("SELECT COUNT(*) FROM followers_by_user WHERE username =?")
    _PREPARED['ADD_FOLLOWING'] = sess.prepare("INSERT INTO following_by_user (username, following_username, is_celebrity) VALUES (?,?,?)")
    _PREPARED['GET_FOLLOWING'] = sess.prepare("SELECT following_username, is_celebrity FROM following_by_user WHERE username =?")
    # Zmieniamy GET_CELEBRITY_POSTS, aby zwracało też post_id do sortowania
    _PREPARED['GET_CELEBRITY_POSTS'] = sess.prepare("SELECT content, post_id FROM posts_by_user WHERE username =? LIMIT 10")
